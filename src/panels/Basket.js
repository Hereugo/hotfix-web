...
const Basket = ({ match: { params: { areaId, itemId }}, foodAreas, order }) => {
  const [ selfService, setSelfService ] = useState(localStorage.getItem('checked'));
  if (selfService === null) setSelfService(1);
  const [ time, setTime ] = useState(localStorage.getItem('time'));
  const [ faster, setFaster ] = useState((time <= 0));
    ...
    <div className="Place">
      ...
      <div className="Place__choice">
        ...
        <div className="Place__choice-item">
          <h3>С собой</h3>
          <Checkbox 
            checked={(selfService == 1)} 
            onToggle={() => {
                localStorage.setItem('checked', selfService^1)
                setSelfService(selfService^1)
              }
            }
          />
        </div>
